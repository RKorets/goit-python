...
def normalize(name):
    dictionary = {ord('А'): 'A', ord('а'): 'a', ord('Q'): 'Q', ord('q'): 'q',
    ...
    translated = []
    for el in list(name):
        if dictionary.get(ord(el)) is None:
            translated.append("_")
        else:
            translated.append(dictionary.get(ord(el)))
...
def filterDir(path):
    ...
    print(f'sort documents in {path}')
...
def recursion(path):
    if path.is_dir():
        for element in path.iterdir():
            recursion(element)
        if path.name not in ["images", "audio", "archives", "documents", "video"]:
...
def start_threading():
    ...
    if len(sys.argv) < 2:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            list(executor.map(filterDir, arguments))
        print(f'end {time() - times}')
